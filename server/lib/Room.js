...
class Room extends EventEmitter {
  ...
  async _handleProtooRequest(peer, request, accept, reject) {
    switch (request.method) {
      ...
      case "startConsumerRecording": {
        const { audioConsumer, videoConsumer, videoConsumerShare } =
          request.data;
        const plainTransportOptions = {
          ...config.mediasoup.plainTransportOptions,
          rtcpMux: false,
        ...
        if (audioConsumer) {
          const rtpTransport = await this._mediasoupRouter.createPlainTransport(
            plainTransportOptions
          );
          ...
          const rtpConsumer = await rtpTransport.consume({
            producerId: audioConsumer.id,
            rtpCapabilities: this._mediasoupRouter.rtpCapabilities,
            paused: true,
          });
          console.log(">>>>>>>>>>startConsumerRecording");
        }
        if (videoConsumer) {
          const rtpTransport = await this._mediasoupRouter.createPlainTransport(
            plainTransportOptions
          ...
          const rtpConsumer = await rtpTransport.consume({
            producerId: videoConsumer.id,
            rtpCapabilities: this._mediasoupRouter.rtpCapabilities,
            paused: true,
          });
        }
        this._startRecordingFfmpeg();
        if (audioConsumer) {
